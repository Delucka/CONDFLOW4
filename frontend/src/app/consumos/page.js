        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            <div className="md:col-span-5">
              ...
              {form.concessionaria === 'Outra' && (
                <input value={form.concessionaria_outra} onChange={e => setForm({...form, concessionaria_outra: e.target.value})}
                  ...
                  className="w-full mt-2 bg-slate-100 border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-800 outline-none focus:border-violet-500" />
              )}
            </div>
          ...
          <div>
            ...
            <input type="file" accept="application/pdf,image/*" onChange={handleFileChange}
              className="mt-1 block w-full text-sm text-slate-700 file:mr-3 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-bold file:bg-violet-500/10 file:text-violet-700 hover:file:bg-violet-500/20" />
            ...
            {duplicatas.length > 0 && (
              <div className="mt-2 p-3 rounded-lg bg-amber-500/10 border border-amber-500/30">
                <p className="text-[11px] font-bold text-amber-700 mb-1 flex items-center gap-1"><AlertTriangle className="w-3.5 h-3.5" /> Arquivo idêntico já enviado:</p>
                <ul className="text-[11px] text-amber-700 list-disc pl-5">
          ...
          {/* Flag conta repetida */}
          <label className="flex items-center gap-3 cursor-pointer p-3 rounded-lg bg-slate-100/50 border border-slate-200">
          ...
          <div className="flex items-center gap-3 pt-2">
            ...
            <div className="flex justify-end gap-3 ml-auto">
              <button type="button" onClick={onClose} className="px-4 py-2 rounded-lg text-sm font-bold bg-slate-100 text-slate-700 hover:bg-slate-200 transition-colors">Cancelar</button>
              ...
              {todosCondos.length === 0 ? (
                ...
                todosCondos.map(c => (
                  <button key={c.id} onClick={() => {
                    ...
                    className="w-full text-left px-4 py-3 rounded-xl bg-slate-50 hover:bg-violet-500/10 hover:border-violet-500/30 border border-slate-200 text-sm text-slate-800 transition-colors">
                    {c.name}
                  </button>
                ))
              )}
                ...
                {lista.map((u, i) => {
                  ...
                  return (
                    <tr key={`${u.apto}-${i}`} className={`border-t border-slate-200 ${anomala ? 'bg-amber-500/10' : 'hover:bg-slate-100'}`}>
                      ...
                      <td className={`px-2 py-1.5 text-right font-mono font-bold ${anomala ? 'text-amber-700' : 'text-violet-700'}`}>
