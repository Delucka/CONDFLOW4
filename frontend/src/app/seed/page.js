...
export default function CleanDuplicatesPage() {
  ...
  return (
    <div className="p-10 space-y-6">
      <h1 className="text-2xl font-bold text-slate-800">Limpeza de Duplicados</h1>
      ...
      <button 
        onClick={handleClean} 
        disabled={loading}
        className="px-6 py-3 bg-rose-500 hover:bg-rose-400 text-white font-bold rounded-lg shadow disabled:opacity-50 transition-colors"
      >
      ...
      <ul className="space-y-2 mt-6 max-h-96 overflow-y-auto">
        {results.map((msg, i) => (
          <li key={i} className="text-sm text-slate-700 font-mono whitespace-pre-wrap">
            {msg}
          </li>
        ))}
      </ul>
