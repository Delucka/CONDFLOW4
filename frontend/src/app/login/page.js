  ...
  return (
    <div className="min-h-screen bg-white flex flex-col items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="glass-panel w-full max-w-md rounded-2xl p-8 animate-fade-up relative z-10">
        ...
        {/* Error */}
        {erro && (
          <div className="mb-6 bg-rose-500/10 border border-rose-500/20 p-4 rounded-xl animate-fade-in">
            <p className="text-sm text-rose-400 font-semibold text-center">{erro}</p>
          </div>
        )}
        ...
        {mode === 'login' && (
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button type="button" onClick={() => { setErro(''); setMode('forgot'); }}
              className="w-full text-xs text-slate-500 hover:text-violet-400 font-bold uppercase tracking-widest transition-colors flex items-center justify-center gap-1.5 pt-2">
        ...
        {mode === 'forgot' && (
          <form onSubmit={handleForgot} className="space-y-5">
            ...
            <button type="button" onClick={() => { setErro(''); setMode('login'); }}
              className="w-full text-xs text-slate-500 hover:text-slate-700 font-bold uppercase tracking-widest transition-colors flex items-center justify-center gap-1.5 pt-2">
        ...
        {mode === 'sent' && (
          <div className="text-center space-y-5 animate-fade-in">
            ...
            <div>
              <h3 className="text-lg font-black text-slate-900 mb-2">E-mail enviado!</h3>
              <p className="text-sm text-slate-400 leading-relaxed">
                Enviamos um link de redefinição para <strong className="text-violet-400">{resetEmail}</strong>.
            ...
            <button onClick={() => { setMode('login'); setResetEmail(''); }}
              className="w-full py-3 bg-slate-100 border border-slate-700 text-slate-700 rounded-xl text-sm font-bold hover:bg-slate-700 transition-all flex items-center justify-center gap-2">
