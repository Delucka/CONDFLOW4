...
export default function ModalSelecionarConta({ planoId, onSelect, onClose, selectedId }) {
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center bg-slate-900/40 backdrop-blur-md p-4 animate-fade-in">
      <div className="bg-white border border-slate-200 rounded-3xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col">
        {/* Header */}
        <div className="px-6 py-5 border-b border-slate-200 flex items-center justify-between shrink-0">
          <div>
            <h3 className="text-lg font-black text-slate-900 uppercase tracking-tight">Selecionar Conta Contábil</h3>
          ...
          <button onClick={onClose} className="text-slate-500 hover:text-slate-900 p-2"><X className="w-5 h-5" /></button>
        </div>

        {/* Busca + toggles */}
        <div className="px-6 py-4 border-b border-slate-200 space-y-3 shrink-0">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-3 bg-white border border-slate-200 rounded-xl text-sm text-slate-800 placeholder:text-slate-600 focus:border-violet-500 outline-none transition-all"
            />
          </div>
          <label className="flex items-center gap-2 cursor-pointer select-none">
            ...
            />
            <span className="text-xs text-slate-500">Mostrar só sintéticas (esconder analíticas)</span>
          </label>
        </div>
          ...
          {loading ? (
            ...
            <div className="py-2">
              {arvoreFiltrada.map(grupo => (
                <div key={grupo.id} className="border-b border-slate-200 last:border-b-0">
                  {/* GRUPO (1º grau) */}
                  <button
                    onClick={() => toggle(grupo.id)}
                    className="w-full flex items-center gap-2 px-6 py-3 hover:bg-slate-100 transition-colors text-left"
                  >
                    {expandidos[grupo.id] ? <ChevronDown className="w-4 h-4 text-violet-400" /> : <ChevronRight className="w-4 h-4 text-slate-500" />}
                    ...
                    <span className="text-sm font-black text-slate-900 uppercase tracking-tight ml-2">{grupo.nome}</span>
                  ...
                  {/* SUBCONTAS */}
                  {expandidos[grupo.id] && (
                    <div className="bg-slate-100 pl-6">
                      {grupo.filhos.map(sint => {
                        ...
                        return (
                          <div key={sint.id}>
                            <div
                              className={`flex items-center gap-2 px-4 py-2 hover:bg-violet-500/5 cursor-pointer transition-colors border-l-2 ${
                                ehSelecionada ? 'border-violet-400 bg-violet-500/10' : 'border-transparent'
                              }`}
                            >
                              <div onClick={() => temAnaliticas && toggle(sint.id)} className="shrink-0">
                                {temAnaliticas
                                  ? (expandidos[sint.id] ? <ChevronDown className="w-3 h-3 text-violet-500/70" /> : <ChevronRight className="w-3 h-3 text-slate-400" />)
                              ...
                              >
                                ...
                                <span className="text-[10px] font-mono text-violet-400 tracking-wider shrink-0">
                                  {codigoFmt(sint)}
                                </span>
                                <span className="text-xs text-slate-700 truncate">{sint.nome}</span>
                                <span className="ml-auto text-[9px] font-bold text-violet-400 bg-violet-500/10 px-2 py-0.5 rounded shrink-0">
                                  #{sint.codigo_reduzido}
                                </span>
                                {ehSelecionada && <Check className="w-3.5 h-3.5 text-violet-400 shrink-0" />}
                              </button>
                            </div>

                            {/* ANALÍTICAS */}
                            {temAnaliticas && expandidos[sint.id] && (
                              <div className="bg-slate-100 pl-6 border-l border-slate-200">
                                {sint.filhos.map(ana => {
                                  ...
                                  return (
                                    <button
                                      ...
                                      className={`w-full flex items-center gap-2 px-4 py-1.5 hover:bg-violet-500/5 transition-colors text-left border-l-2 ${
                                        ehSelAna ? 'border-violet-400 bg-violet-500/10' : 'border-transparent'
                                      }`}
                                    >
                                      <span className="w-3" />
                                      <span className="text-[10px] font-mono text-violet-400/80 tracking-wider shrink-0">
                                        {codigoFmt(ana)}
                                      </span>
                                      <span className="text-[11px] text-slate-400 italic truncate">{ana.nome}</span>
                                      <span className="ml-auto text-[9px] font-bold text-violet-300 bg-violet-500/10 px-2 py-0.5 rounded shrink-0">
                                        #{ana.codigo_reduzido}
                                      </span>
                                      {ehSelAna && <Check className="w-3.5 h-3.5 text-violet-400 shrink-0" />}
                                    </button>
                                  );
                                })}
                              </div>
                      ...
                      {grupo.filhos.length === 0 && (
                        <div className="px-4 py-2 text-[10px] text-slate-400 italic">Nenhuma subconta neste grupo</div>
                      )}
                    </div>
                  )}
                </div>
        ...
        {/* Footer */}
        <div className="px-6 py-3 border-t border-slate-200 flex items-center justify-between shrink-0 text-[10px] text-slate-500 font-bold uppercase tracking-widest">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded bg-violet-400" /> Sintética</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded bg-violet-400" /> Analítica</span>
          </div>
        </div>
      </div>
    </div>
