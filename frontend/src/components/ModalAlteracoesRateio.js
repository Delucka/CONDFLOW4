...
const COLORS = {
  ...
  orange:  { bg: 'bg-amber-500/10',  border: 'border-amber-500/30',  text: 'text-amber-300',  dot: 'bg-amber-400' },
  cyan:    { bg: 'bg-violet-500/10',    border: 'border-violet-500/30',    text: 'text-violet-300',    dot: 'bg-violet-400' },
  emerald: { bg: 'bg-emerald-500/10', border: 'border-emerald-500/30', text: 'text-emerald-300', dot: 'bg-emerald-400' },
  slate:   { bg: 'bg-slate-500/10',   border: 'border-slate-500/30',   text: 'text-slate-700',   dot: 'bg-slate-400' },
};
...
export default function ModalAlteracoesRateio({ condoId, ano, mesInicial = null, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-900/40 backdrop-blur-md p-4 animate-fade-in">
      <div className="bg-white border border-slate-200 rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto">
          {/* FORM */}
          {mostrarForm && (
            <form onSubmit={handleSubmit} className="p-6 border-b border-slate-200 space-y-4 bg-slate-50">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Mês</label>
                  <select required value={form.mes} onChange={(e) => setForm({ ...form, mes: parseInt(e.target.value) })}
                    className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-800 outline-none focus:border-amber-500">
              ...
              <div>
                <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Descrição (opcional)</label>
                <textarea value={form.descricao} onChange={(e) => setForm({ ...form, descricao: e.target.value })}
                  rows={2} placeholder="Ex: Aprovação da reforma da piscina"
                  className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-800 outline-none focus:border-amber-500 placeholder:text-slate-700" />
              </div>
              ...
              <div className="flex items-center justify-end gap-3 pt-2">
                <button type="button" onClick={fecharForm} className="px-4 py-2 text-xs text-slate-500 font-bold uppercase tracking-widest hover:text-slate-900 transition-colors">
                  Cancelar
                </button>
                    ...
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="flex flex-col items-end gap-1.5">
                        ...
                        <div className="flex gap-1 mt-1">
                          <button onClick={() => abrirForm(alt)} className="p-1.5 text-slate-500 hover:text-violet-400" title="Editar">
        ...
        <div className="px-6 py-3 border-t border-slate-200 text-[10px] text-slate-400 uppercase tracking-widest font-bold flex items-center gap-2 shrink-0">
