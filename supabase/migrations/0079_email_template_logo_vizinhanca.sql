-- ==========================================
-- MIGRATION 0079: Cabeçalho do e-mail passa a usar a marca nova ("Vizinhança")
-- Idêntica à 0072, trocando apenas a imagem: /email-penguin.gif -> /email-logo.png
-- (o conjunto de condomínios que substituiu o mascote pinguim).
-- O GIF antigo continua no servidor de propósito, para não quebrar a imagem dos
-- e-mails que já foram enviados.
-- ==========================================
...
RETURNS text LANGUAGE sql IMMUTABLE AS $$
  SELECT
    ...
    -- cabeçalho navy + marca
    || '<div style="background:#142a63;padding:18px 24px;">'
    ||   '<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%"><tr>'
    ||     '<td style="vertical-align:middle;width:48px;"><img src="https://emissaonline.com/email-logo.png" width="48" height="48" alt="" style="display:block;border-radius:14px;border:0;"></td>'
    ...
    ||   CASE WHEN p_link IS NOT NULL AND p_link <> '' THEN
           '<div style="margin-top:22px;"><a href="https://emissaonline.com' || p_link || '" style="display:inline-block;background:#142a63;color:#ffffff;font-size:15px;font-weight:bold;padding:13px 30px;border-radius:10px;text-decoration:none;">Abrir no CondoFlow</a></div>'
         ELSE '' END
    || '</div>'
    -- rodapé corporativo
