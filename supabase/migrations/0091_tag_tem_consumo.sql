...
-- fatura e relatorio de concessionaria. Hoje isso vive na cabeca de quem faz.
--
...
-- condominios e nao e a lista que a operacao usa hoje. Serve para a tela de
-- /consumos; nao serve como "tem ou nao tem".
--
-- Esta migration cria a resposta direta -- um booleano em "condominios" -- a
-- partir da relacao passada pela operacao em 05/08/2026. As duas coisas
-- convivem: o booleano diz SE tem, a tabela diz QUAIS, quando se sabe.
--
-- O casamento e pelo prefixo numerico do nome, comparado como NUMERO: assim
-- "088 DUQUE" acha tanto "088 - ..." quanto "88 - ...". E a mesma tecnica do
-- seed da 0036 e do cadastro de seguros.
--
...
CREATE TEMP TABLE _consumo_lista (codigo integer PRIMARY KEY, apelido text);

INSERT INTO _consumo_lista (codigo, apelido) VALUES
  (88, 'DUQUE DI FAUSTUS'),
  (94, 'GIOVANNI PASCOLI'),
  (111, 'VILLE MARSEILLE'),
  (127, 'GREGÓRIO SERRÃO'),
...
UPDATE public.condominios c
  ...
  FROM _consumo_lista l
 WHERE substring(c.name from '^[[:space:]]*([0-9]+)')::int = l.codigo;

-- ---- Conferencia (roda junto, no mesmo lote) ----

-- 1. Quantos ficaram marcados. Esperado: perto de 119.
SELECT count(*) AS marcados FROM public.condominios WHERE tem_consumo;

-- 2. Codigos da lista que NAO acharam condominio -- erro de digitacao ou
--    condominio que saiu da base. Esperado: nenhuma linha.
...
-- 3. Tinha concessionaria cadastrada mas NAO esta na lista nova. Nada foi
--    apagado -- e so para conferir se algum ficou de fora sem querer.
